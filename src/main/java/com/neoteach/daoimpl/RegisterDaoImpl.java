...
////			{
//			Query query=session.createSQLQuery("insert into tutors(firstname,lastname,gender,mobile,qualification,subject,leveltoteach,location,description,regDate)" +
...
////			if(result!=0)
////			{
////				mailsendUtil.sendMail(regform.getUsername(), "<h2>Dear\t"+regform.getFirstName()+",\tThanks For Registration with NeoHomeTuitions as a Learner.</h2>", "Student Registration");
////				
////			}
////			}
////			else{
////				Query query=session.createSQLQuery("insert into mtutors(firstname,lastname,gender,mobile,qualification,subject,leveltoteach,location,description,regDate)" +
////			            "values('"+regform.getFirstname()+"','"+regform.getLastname()+"','"+regform.getGender()+"','"+regform.getMobile()+"','"+regform.getQualification()+"','"+regform.getSubject()+"','"+regform.getLeveltoteach()+"','"+regform.getLocation()+"','"+regform.getDescription()+"','"+regDate+"')");
////	query.executeUpdate();
////	transaction.commit();
////	result=1;
...
//		try
//		{
//			Query query=session.createSQLQuery("select IFNULL(firstname,'-'),IFNULL(lastname,'-'),IFNULL(mobile,'-') from tutors where mobile='"+phone+"'");
...
////			if(regform.getGender().equalsIgnoreCase("F"))
////			{
////			Query query=session.createSQLQuery("select IFNULL(firstname,'-'),IFNULL(lastname,'-'),IFNULL(mobile,'-') from ftutors where mobile='"+phone+"'");
//////			List list=query.list();
////			List list= new ArrayList();
////			list=query.list();
////			if(list.size()!=0)
...
////			{
////
////				Query query=session.createSQLQuery("select IFNULL(firstname,'-'),IFNULL(lastname,'-'),IFNULL(mobile,'-') from mtutors where mobile='"+phone+"'");
//////				List list=query.list();
////				List list= new ArrayList();
////				list=query.list();
////				if(list.size()!=0)
